...
interface FeedbackWithActivity extends JeckieFeedback {
  activity?: JeckieActivity;
}

export const addFeedback = async (feedback: Omit<JeckieFeedback, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  try {
...
export const getFeedbackForActivity = async (activityId: string): Promise<JeckieFeedback | null> => {
  try {
    const { data, error } = await supabase
      .from('sales_feedback')
      .select('*')
