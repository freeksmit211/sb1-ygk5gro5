...
const JeckieYearToDate: React.FC = () => {
  ...
  if (!budget || !budget.accounts.jeckie) {
    return (
  ...
  const jeckieData = budget.accounts.jeckie.monthlyData[currentMonth] || {
    target: 0,
    invoiceValue: 0,
    orderValue: 0
  };

  const achievementPercentage = jeckieData.target > 0 
    ? (jeckieData.invoiceValue / jeckieData.target) * 100 
    : 0;
  ...
  const getCurrentMonthFuelUsage = () => {
    ...
    fuelEntries.forEach(entry => {
      ...
      if (
        ...
        entry.driver.toLowerCase() === 'jeckie'
      ) {
        liters += entry.liters;
  ...
  // Create a modified budget object with only Jeckie's data
  const jeckieBudget = {
    year: budget.year,
    accounts: {
      jeckie: budget.accounts.jeckie
    }
  };
    ...
    <div className="min-h-screen bg-blue-600 p-4 md:p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="space-y-2">
          <h1 className="text-2xl font-bold text-white">Jeckie's Overview</h1>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(jeckieData.target)}</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600 mt-1">{formatCurrency(jeckieData.invoiceValue)}</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-2xl font-bold text-orange- 600 mt-1">{formatCurrency(jeckieData.orderValue)}</p>
            </div>
      ...
      <LastThreeMonthsPerformance budget={jeckieBudget} />
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <CurrentMonthGraph budget={jeckieBudget} />
      </div>
      ...
      <YearToDateFuelUsage entries={fuelEntries} repId="jeckie" />
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <TodoBoard repId="jeckie" />
      </div>
    </div>
  );
};

export default JeckieYearToDate;