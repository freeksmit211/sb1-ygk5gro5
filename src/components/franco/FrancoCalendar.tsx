...
import FrancoActivityDetailsModal from './FrancoActivityDetailsModal';
import FrancoFeedbackList from './FrancoFeedbackList';
import { getFrancoActivitiesByDate, deleteFrancoActivity } from '../../services/francoActivityService';
import { FrancoActivity } from '../../types/francoActivity';
import VoiceControl from './VoiceControl';
import { addFeedback } from '../../services/francoFeedbackService';

const FrancoCalendar: React.FC = () => {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState<Date | null>(null);
  const [selectedActivity, setSelectedActivity] = useState<FrancoActivity | null>(null);
  const [activities, setActivities] = useState<Record<string, FrancoActivity[]>>({});
  ...
  const loadActivities = async () => {
    try {
      ...
      const monthActivities = await getFrancoActivitiesByDate(startOfMonth, endOfMonth);
      
      const grouped = monthActivities.reduce((acc, activity) => {
        const date = new Date(activity.date).toISOString().split('T')[0];
        acc[date] = [...(acc[date] || []), activity];
        return acc;
      }, {} as Record<string, FrancoActivity[]>);
  ...
  const handleEdit = (activity: FrancoActivity, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleReschedule = (activity: FrancoActivity, e: React.MouseEvent) => {
    e.stopPropagation();
    setSelectedActivity(activity);
    setSelectedDate(new Date(activity.date));
  ...
  const handleDelete = async (activity: FrancoActivity, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    try {
      setError(null);
      await deleteFrancoActivity(activity.id);
  ...
  const handleConvertToFeedback = async (activity: FrancoActivity, e: React.MouseEvent) => {
    e.stopPropagation();
    setSelectedActivity(activity);
  ...
  const getActivityColor = (type: FrancoActivity['type']) => {
    switch (type) {
      case 'meeting':
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold text-gray-900">Franco's Activities</h2>
          ...
          <div className="flex items-center gap-4">
            {/* Voice Control */}
            <VoiceControl onAddActivity={handleDateClick} />
