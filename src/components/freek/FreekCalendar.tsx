...
import FreekActivityDetailsModal from './FreekActivityDetailsModal';
import FreekFeedbackList from './FreekFeedbackList';
import { getFreekActivitiesByDate, deleteFreekActivity } from '../../services/freekActivityService';
import { FreekActivity } from '../../types/freekActivity';
import VoiceControl from './VoiceControl';
import { addFeedback } from '../../services/freekFeedbackService';

const FreekCalendar: React.FC = () => {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState<Date | null>(null);
  const [selectedActivity, setSelectedActivity] = useState<FreekActivity | null>(null);
  const [activities, setActivities] = useState<Record<string, FreekActivity[]>>({});
  ...
  const loadActivities = async () => {
    try {
      ...
      const monthActivities = await getFreekActivitiesByDate(startOfMonth, endOfMonth);
      
      const grouped = monthActivities.reduce((acc, activity) => {
        const date = new Date(activity.date).toISOString().split('T')[0];
        acc[date] = [...(acc[date] || []), activity];
        return acc;
      }, {} as Record<string, FreekActivity[]>);
  ...
  const handleEdit = (activity: FreekActivity, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleReschedule = (activity: FreekActivity, e: React.MouseEvent) => {
    e.stopPropagation();
    setSelectedActivity(activity);
    setSelectedDate(new Date(activity.date));
  ...
  const handleDelete = async (activity: FreekActivity, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleConvertToFeedback = async (activity: FreekActivity, e: React.MouseEvent) => {
    e.stopPropagation();
    setSelectedActivity(activity);
  ...
  const getActivityColor = (type: FreekActivity['type']) => {
    switch (type) {
      case 'meeting':
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold text-gray-900">Freek's Activities</h2>
