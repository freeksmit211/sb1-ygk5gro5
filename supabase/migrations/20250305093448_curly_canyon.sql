...
DROP POLICY IF EXISTS "Enable read access for order notes" ON order_notes;
DROP POLICY IF EXISTS "Enable insert access for order notes" ON order_notes;
DROP POLICY IF EXISTS "Enable read access for repair notes" ON repair_notes;
DROP POLICY IF EXISTS "Enable insert access for repair notes" ON repair_notes;
...
DROP INDEX IF EXISTS idx_repair_notes_repair_id;

-- Create indexes
CREATE INDEX IF NOT EXISTS idx_order_notes_order_id ON order_notes(order_id);
CREATE INDEX IF NOT EXISTS idx_repair_notes_repair_id ON repair_notes(repair_id);

-- Drop existing triggers if they exist
DROP TRIGGER IF EXISTS update_order_notes_updated_at ON order_notes;
DROP TRIGGER IF EXISTS update_repair_notes_updated_at ON repair_notes;
