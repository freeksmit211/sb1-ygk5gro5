...
DROP VIEW IF EXISTS license_renewal_alerts;

-- Drop the constraint from sheq_vehicles first
ALTER TABLE sheq_vehicles
DROP CONSTRAINT IF EXISTS unique_vehicle_registration;

-- Drop the index from sheq_vehicles
DROP INDEX IF EXISTS unique_vehicle_registration;

-- Create a backup of existing data
CREATE TEMP TABLE temp_vehicle_licenses AS
SELECT * FROM vehicle_licenses;

-- Drop and recreate the table with proper constraints
DROP TABLE vehicle_licenses CASCADE;
...
SELECT
  ...
  created_at,
  updated_at
FROM temp_vehicle_licenses;

-- Drop temporary table
DROP TABLE temp_vehicle_licenses;
